...
- 安全:[API]禁止上传 SVG 图片以避免被植入脚本造成存储型 XSS 风险
  - 影响范围:后台媒体库上传、前台通用图片上传、前台用户头像上传
  - 升级提示:升级后已上传的历史 SVG 文件仍可访问,建议运维侧巡检 `media_images` 表中 `name`/`path`/`url` 含 `.svg` 的记录并按需下线
...
- 修复:[Admin]管理员角色权限选择器在小屏幕设备几乎无法操作的问题
- 修复:[Admin]PC 装修预览区首个板块(幻灯片等)的操作条被容器 overflow 裁切,导致无法点击删除/复制按钮的问题
...
- 新增:[PC|H5]课程详情和视频播放页面新增已学习进度显示
- 新增:[ALL]装修页面新增页面功能
- 新增:[Admin]学员批量删除
- 新增:[H5]公告全局顶部显示
- 新增:[API|Admin]优惠码批量导入新增可使用次数字段
- 新增:[API|Admin]优惠码列表新增生效中/已用完/已过期状态的筛选查询
- 新增:[API|Admin]优惠码使用明细
- 新增:[Admin]已使用优惠码禁止删除
- 修复:[API]微信公众号授权登录获取头像失败的 BUG
- 修复:[Admin]优惠码批量导入模板本地下载
- 修复:[API]全局导航查询缓存 Bug
- 修复:[API]友情链接查询缓存 Bug
- 修复:[H5]图片魔方渲染样式 Bug
- 修复:[API]单元测试报错
- 优化:[H5]视频播放器按照16/9的比率渲染高度
- 优化:[PC|H5]视频播放接口请求失败的错误友好提示
- 优化:[PC|H5]视频播放地址为空的友好提示
- 优化:[API]重构后台权限校验
- 优化:[PC]重构个人中心布局
- 优化:[PC|Admin]幻灯片修改为装修的一个模块
- 优化:[PC]公告全局顶部显示
- 优化:[Admin|API]后台权限按照后台左侧菜单进行分组和排序
- 优化:[API]删除幻灯片功能
- 修复:[API]删除用户会自动清理协议同意记录
- 修复:[Admin]因用户删除导致用户协议同意列表白屏的 BUG
