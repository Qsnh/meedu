...
class ImageGroupHTMLHook implements HookRuntimeInterface
{
    public function handle(HookParams $params, \Closure $closure)
    {
        $block = $params->getValue('block');

        if (!in_array($block['sign'], [Constant::H5_BLOCK_IMAGE_GROUP])) {
            return $closure($params);
        }

        $html = app('Illuminate\View\Factory')->file(resource_path('views/h5/view-block/image-group.blade.php'), ['block' => $block])->render();
